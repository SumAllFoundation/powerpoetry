...
if __name__ == "__main__":
	...
	# {
	#     "content": "Wir lernen Deutsch. Deutsch lernen wir"
	# }
	...
	class yhat_poetry(YhatModel):
		#Deploy Requirements
		REQUIREMENTS = reqs
		#Question --> preprocess converts the incoming dict onto a DataFrame? 
		@preprocess(in_type=pd.DataFrame, out_type = dict)
		def execute(self,data):
			content = data['content']
			#Sentiment
			sentiment_features = sentiment(dbparams,content)
			ABS,EnlTot,Female,Male,Object,Polit,Race,Relig,St,WlbPhycs,WlbPsyc,PosNeg = sentiment_features
			#NGrams
			ngram_features = ngram(dbparams,content)
			unigramFreq,bigramFreq,trigramFreq,misspeltWord, sentence_count,logWordCount,punctFreq = ngram_features
			#Sound
			sound_features = sound(content)
			perfectRhymeFreq,slantRhymeFreq,alliterFreq = sound_features
			#Output - Just few features
			features =DataFrame([perfectRhymeFreq,slantRhymeFreq,Polit,Race,Relig,PosNeg,unigramFreq,bigramFreq,trigramFreq,misspeltWord, sentence_count,logWordCount,punctFreq]).transpose()
